...
export const AppProvider = ({ children }) => {
  ...
  }, [isSidebarOpen]); // Dependencia en isSidebarOpen para useCallback
  ...
  // *** ELIMINAMOS ESTE useEffect PROBLEMÁTICO ***
  // useEffect(() => {
  ...
  // NOTA: La lógica para cerrar el sidebar en móvil al cambiar de pestaña
  // debe implementarse en el manejador de clic de los elementos del menú/pestañas
  // que llaman a setActiveTab.
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      // Esto evita que el sidebar quede abierto si redimensionas de desktop a móvil
      if (isMobile && isSidebarOpen) {
        setIsSidebarOpen(false);
      }
       // Si cambiamos a desktop y el sidebar está cerrado, lo abrimos (opcional, según diseño)
       // Puedes añadir lógica aquí si quieres que el sidebar siempre esté abierto en desktop
       // if (!isMobile && !isSidebarOpen) {
    ...
    return cleanup;
  }, [isSidebarOpen, setIsSidebarOpen]); // Dependencia en isSidebarOpen y su setter


  // Estados principales (mantienen la lógica de carga desde localStorage)
  ...
  // Estados de modales (mantienen la lógica original)
  ...
  // Menú contextual (mantiene la lógica original)
  ...
  // Menú de salones (mantiene la lógica original)
  const [showSalonMenu, setShowSalonMenu] = useState(false);

  // Integración con la simulación de tiempo real (mantiene la lógica original)
  const { lastUpdate, isUpdating } = useRealTimeUpdates(tables, setTables);

  // Sincronización con localStorage (mantienen la lógica original)
  useEffect(() => {
    try {
  ...
  // Sincronización con WooCommerce (mantienen la lógica original)
  useEffect(() => {
  ...
  // Filtrar mesas por salón activo (mantiene la lógica original)
  const filteredTables = tables.filter(table => table.salon === activeSalon);

  // Helper function to get orders for a specific table (mantiene la lógica original)
  const getTableOrders = (tableId) => {
    return orders.filter(order => order.tableId === tableId);
  };

  // Helper function to calculate total for a table (mantiene la lógica original)
  ...
  // Estadísticas para el salón activo (mantiene la lógica original)
  const salonStats = {
  ...
  // Manejadores de datos (mantienen la lógica original)
  ...
  // Cerrar el menú contextual al hacer clic fuera (mantiene la lógica original)
  useEffect(() => {
  ...
  const contextValue = {
    ...
    filteredTables,
    salonStats,
    isSidebarOpen, // Estado para el sidebar
    ...
    setProducts,
    setCategories,
    setActiveTab, // Asegúrate de que al llamar a esta función en móvil, cierres el sidebar
    setActiveSalon,
    setViewMode,
    setViewType,
    setShowTableModal,
    ...
    setShowContextMenu,
    setShowSalonMenu,
    setIsSidebarOpen, // Setter para el sidebar
